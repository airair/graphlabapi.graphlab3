...
namespace graphlab {
  ...
  namespace dc_impl {
    ...
    void dc_tcp_comm::init(const std::vector<std::string> &machines,
                           ...
                           int curmachineid,
                           std::vector<dc_stream_receive*> receiver_,
                           std::vector<dc_buffered_stream_send2*> sender_) {

      curid = curmachineid;
      ASSERT_LT(machines.size(), std::numeric_limits<int>::max());
      nprocs = (int)(machines.size());
      receiver = receiver_;
      sender = sender_;
    ...
    void dc_tcp_comm::trigger_send_timeout(int target, bool urgent) {
      if (!urgent) {
...
////////////////////////////////////////////////////////////////////////////
    ...
    void dc_tcp_comm::accept_handler() {
      ...
      while(numsocks_connected < sock.size()) {
        ...
        if (pf.revents & POLLIN) {
          ...
          int remotemachineid = (int)(-1);
          ssize_t msglen = 0;
          while(msglen != sizeof(int)) {
    ...
    void on_receive_event(int fd, short ev, void* arg) {
      ...
      if (ev & EV_READ) {
        ...
        dc_stream_receive* receiver = comm->receiver[sockinfo->id];
