...
key_value::key_value(std::vector<std::string> zkhosts, 
    ...
    prefix(_prefix), has_callback(false), callback(NULL) {
...
void key_value::get_all_keys_locked(
  ...
  print_stat(stat, "zk_key_value get_all_keys", values_path);
  std::vector<std::string> allkeys = String_vector_to_vector(&children);
  free_String_vector(&children); 

  fill_data_locked(allkeys, out_newkeys, out_deletedkeys, out_modifiedkeys);
}
...
void key_value::fill_data_locked(const std::vector<std::string>& keys,
  ...
  for (size_t i = 0;i < keys.size(); ++i) {
    ...
    key_and_version[keyname] = version;
  }
  ...
  // modified keys
  {
    ...
    while (iter != key_and_version.end()) {
      ...
      if (data_iter == data.end()) {
        ...
        data_iter->second.remote_version = iter->second;
        data[iter->first].has_value = (data[iter->first].stored_version == data[iter->first].remote_version);
      }
      ++iter;
    }
...
void key_value::set_callback(
    boost::function<void(key_value*,
                         const std::vector<std::string>& out_newkeys,
                         const std::vector<std::string>& out_deletedkeys,
                         const std::vector<std::string>& out_modifiedkeys)> fn) {
  datalock.lock();
  callback = fn;
  has_callback = (fn != NULL);
  datalock.unlock();
}
...
void key_value::watcher(zhandle_t *zh, 
  ...
  if (type == ZOO_CHILD_EVENT) {
    ...
    slist->get_all_keys_locked(newkeys, deletedkeys, modifiedkeys);
    if (slist->has_callback) slist->callback(slist, newkeys, deletedkeys, modifiedkeys);
