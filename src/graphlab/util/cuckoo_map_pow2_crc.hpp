    ...
    iterator do_insert(const value_type& v_) {
      ...
      for (int i = 0;i < 100; ++i) {
        ...
        for (size_t j = 0; j < CuckooK; ++j) {
          idx = compute_hash_inc(hash_of_k, acc, j);
  ...
  public:
...
#endif
      size_t acc = 0;
      for (uint32_t i = 0;i <= seed; ++i) acc = __builtin_ia32_crc32di(acc, k ^ a[i]); 
      return acc & mask;
    }


    index_type compute_hash_inc(size_t k , size_t& acc, size_t i) const {
      // a bunch of random numbers
#if (__SIZEOF_PTRDIFF_T__ == 8)
      static const size_t a[8] = {0x6306AA9DFC13C8E7,
                                  0xA8CD7FBCA2A9FFD4,
      ...
      static const size_t a[8] = {0xFC13C8E7,
                                  ...
                                  0xBFEC63E9};
#endif
      acc = __builtin_ia32_crc32di(acc, k ^ a[i]); 
      return acc & mask;
    }
    ...
    iterator find(key_type const& k) {
      ...
      for (uint32_t i = 0;i < CuckooK; ++i) {
        index_type idx = compute_hash_inc(hash_of_k, acc, i);
