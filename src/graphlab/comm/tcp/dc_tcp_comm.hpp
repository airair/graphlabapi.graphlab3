...
class dc_tcp_comm {
  ...
  */
  void init(const std::vector<std::string> &machines,
            ...
            int curmachineid,
            std::vector<dc_stream_receive*> receiver,
            std::vector<dc_buffered_stream_send2*> senders);
  ...
  */
  ...
  void trigger_send_timeout(int target, bool urgent);
 ...
 private:
  ...
  void new_socket(int newsock, sockaddr_in* otheraddr, int remotemachineid);
  ...
  int curid;   /// if od the current processor
  ...
  std::map<uint32_t, int> addr2id;
  std::vector<uint16_t> portnums;
  
  std::vector<dc_stream_receive*> receiver;
  std::vector<dc_buffered_stream_send2*> sender;
